...
def build_markdown_report(version_tag, run_name, run_dir, config, train_summary, dev_summary, test_summary):
    ...
    lines = [
        ...
        "- 목적: `problem_generation v3 split-descriptive line`이 만든 `data/processed/aihub/problem_generation/v3_split_descriptive/{train,dev,test}.jsonl`을 직접 소비하는 첫 lightweight real baseline을 잠그는 것",
        "",
        "## 소비 구조",
        "",
        "- 입력: `data/processed/aihub/problem_generation/v3_split_descriptive/{train,dev,test}.jsonl`의 generated problem, focus issue, split meta",
