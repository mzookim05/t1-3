...
## 생성 지시
- 문제 본문은 `1문장`을 기본으로 하고, 정말 필요할 때만 `2문장`까지 허용한다.
- 반드시 하나의 쟁점만 묻는다.
- `법령_QA`인 경우 stem에는 `요건`, `기준`, `절차`, `방식`, `경로`, `효과` 중 하나만 남기고 둘 이상을 함께 쓰지 않는다.
- `4지`를 모두 작성한다.
- 정답은 정확히 하나만 되게 만든다.
...
- 문제 본문 마지막에는 `무엇인가`와 `옳은 설명을 고르시오`를 함께 중복해서 쓰지 않는다.
- 출력은 JSON 객체만 준다.
