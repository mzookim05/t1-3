...
def build_local_fallback_problem(seed):
    ...
    for label, (dtype, text) in zip(distractor_labels, distractors, strict=True):
        choices[label] = text
...
def build_messages(seed):
    system_prompt = load_prompt("generator_system_mcq.txt")
    user_template = load_prompt("generator_user_template_mcq.md")
...
def main(mode="main"):
    snapshot_prompts(["generator_system_mcq.txt", "generator_user_template_mcq.md"])
