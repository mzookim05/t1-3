...
# `pb8`은 법령/해석례/판결문 repair 트랙을 분리한 뒤, 지금까지 가장
# 안정적인 신호를 보인 `결정례_QA`만으로 count 확보 가능성을 확인하는 runner다.
...
VERSION_TAG = "pb8_decision_only_objective_current_r2"
# llm_runs 폴더 정렬을 위해 최초 생성 시각의 HHMMSS까지 run stamp에 고정한다.
RUN_DATE = "2026-04-25_182837"
RUN_PURPOSE = "objective_r2_decision_only_controlled_batch"
...
REFERENCE_PB7_SEED_REGISTRY_PATH = (
    pb6.pb4.pb3.base.PROJECT_ROOT
    / "data"
    / "interim"
    / "aihub"
    / "problem_generation"
    / "production_batches"
    / "pb7_decision_judgment_objective_current_r2"
    / "seed_registry.csv"
)
...
ORIGINAL_CLASSIFY_TAIL = pb6.classify_tail


def collect_excluded_rows():
    # failed batch도 이미 generation/Judge를 본 seed이므로 `pb8` fresh 검증에서는
    # `pb6`, `pb7` seed registry까지 모두 seen pool로 보호한다.
    rows = ORIGINAL_COLLECT_EXCLUDED_ROWS()
    rows.extend(pb6.load_csv_rows_if_exists(REFERENCE_PB6_SEED_REGISTRY_PATH))
    rows.extend(pb6.load_csv_rows_if_exists(REFERENCE_PB7_SEED_REGISTRY_PATH))
    return rows


def passes_pb8_seed_filter(spec, payload):
    if spec["doc_type_name"] != "결정례_QA":
        return False, "non_decision_doc_type_restricted"
...
def classify_pb8_tail(row):
    tags = row.get("error_tags", "")
    # `pb8`은 결정례 전용 batch라, 같은 weak distractor라도 비법령 전체가 아니라
    # decision-only guardrail 대상으로 남겨야 다음 repair 축이 흔들리지 않는다.
    ...
    return ORIGINAL_CLASSIFY_TAIL(row)


def configure_pb8_globals():
    # `pb6`의 strict-final 실행 본체를 유지하고, identity와 source scope만
    # `결정례_QA only` stop line에 맞게 재배선한다.
    pb6.VERSION_TAG = VERSION_TAG
    pb6.RUN_DATE = RUN_DATE
    pb6.RUN_PURPOSE = RUN_PURPOSE
    pb6.RUN_NAME = RUN_NAME
    pb6.RUN_LABEL = "pb8 decision-only"
    pb6.SEED_ID_PREFIX = "pb8_decision"
    pb6.SEED_SELECTION_ROLE = "objective_pb8_decision_only_current_r2_seed"
    pb6.SEED_SELECTION_NOTE = "법령_QA/해석례_QA/판결문_QA repair 분리 상태에서 결정례_QA count 확보를 검증하는 seed"
    pb6.SEED_FILTER_NOTE = "decision_only_seen_seed_pool_excluded"
    pb6.SCOPE_NOTE = "결정례_QA only; 법령_QA, 해석례_QA, 판결문_QA는 repair track 또는 별도 판단"
    pb6.EXPECTED_TOTAL_SEED_COUNT = 40
    pb6.SUCCESS_USABLE_MIN = 38
    pb6.SUCCESS_HARD_FAIL_MAX = 0
    pb6.SUCCESS_SOFT_FAIL_MAX = 0
    pb6.SUCCESS_AUDIT_MAX = 2
    pb6.SUCCESS_LAW_ROW_COUNT = 0
    pb6.CANDIDATE_RECIPE_SOURCE = "v2_difficulty_patch_r2_decision_only_current"
    pb6.SEED_REGISTRY_STRATEGY = "fresh_aihub_qa_training_decision_only_pool_excluding_current_law_targeted_failed_pb5_failed_pb6_failed_pb7_heldout_audit_rows"
    pb6.LAW_STATUS_NOTE = "law_interpretation_judgment_repair_tracks_excluded_from_pb8"
    pb6.OVERLAP_CHECK_LABEL = "no current/law-targeted/failed-pb5/pb6/pb7/held-out/audit overlap"
    pb6.EXCLUSION_WORDING_LINES = [
        ...
        "`law targeted pilot 16개`, failed `pb5 40개`, failed `pb6 45개`, failed `pb7 40개`까지 더해, 이번 결정례 only batch에서는 seen objective seed `250개`를 제외 대상으로 본다.",
    ]
    ...
    pb6.PB6_SOURCE_COUNTS = PB8_SOURCE_COUNTS
    ...
    pb6.passes_pb6_seed_filter = passes_pb8_seed_filter
    pb6.classify_tail = classify_pb8_tail


def main():
    configure_pb8_globals()
    return pb6.main()
