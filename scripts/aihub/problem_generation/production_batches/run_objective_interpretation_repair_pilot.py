...
# reviewer sign-off 이후 같은 16개 해석례 seed를 실제 API 경로로 태우는 runner다.
...
VERSION_TAG = "objective_interpretation_repair_pilot"
# llm_runs 이름은 실제 실행 시각과 맞아야 하므로 run stamp를 자동 생성한다.
RUN_DATE = build_run_stamp()
RUN_PURPOSE = "objective_r2_interpretation_repair_api_pilot"
RUN_NAME = f"{RUN_DATE}_{VERSION_TAG}_{RUN_PURPOSE}"

PROJECT_ROOT = pb6.pb4.pb3.base.PROJECT_ROOT
SOURCE_PREFLIGHT_VERSION_TAG = "objective_interpretation_repair_pilot_seed_preflight"
SOURCE_PREFLIGHT_RUN_PURPOSE = "objective_r2_interpretation_repair_seed_preflight_wiring_check"
SOURCE_PREFLIGHT_INTERIM_DIR = (
    PROJECT_ROOT
    / "data"
    / "interim"
    / "aihub"
...
SOURCE_PREFLIGHT_SEED_REGISTRY_PATH = SOURCE_PREFLIGHT_INTERIM_DIR / "seed_registry.csv"


def read_csv_key_rows_for_provenance(path: Path) -> list[tuple[str, str, str, str, str, str]]:
    # API run stamp와 source preflight run stamp는 서로 달라야 하므로,
    # import 시점의 dynamic RUN_NAME 대신 실제 registry 내용을 비교해 locked preflight를 찾는다.
...
def resolve_source_preflight_run_dir() -> Path:
    # 같은 날 여러 preflight가 생겨도 seed registry가 같은 locked run만 provenance로 채택한다.
    ...
    source_registry_keys = read_csv_key_rows_for_provenance(SOURCE_PREFLIGHT_SEED_REGISTRY_PATH)
    ...
    for candidate_dir in sorted(llm_runs_root.glob(pattern)):
        ...
        if not candidate_registry.exists():
            continue
        if read_csv_key_rows_for_provenance(candidate_registry) == source_registry_keys:
...
SUCCESS_HARD_FAIL_MAX = 1
SUCCESS_SOFT_FAIL_MAX = 1
SUCCESS_AUDIT_MAX = 1

BATCH_STATUS = "interpretation_repair_pilot_not_counted"
...
def write_target_label_schedule(seed_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    rows = [
        {
            ...
            "interpretation_failure_class": row.get("interpretation_failure_class", ""),
    ...
    if dict(counts) != TARGET_LABEL_COUNTS:
        raise RuntimeError(f"interpretation target label schedule mismatch: {dict(counts)}")
...
def assert_fixed_seed_registry(seed_rows: list[dict[str, str]]) -> None:
    ...
    if len(seed_rows) != EXPECTED_TOTAL_SEED_COUNT:
        raise RuntimeError(f"interpretation pilot seed count mismatch: {len(seed_rows)}")
    if dict(doc_counts) != EXPECTED_DOC_TYPE_COUNTS:
        raise RuntimeError(f"interpretation pilot doc type mismatch: {dict(doc_counts)}")
    for key, expected_count in EXPECTED_LANE_BY_DOC.items():
        if lane_by_doc.get(key, 0) != expected_count:
            raise RuntimeError(f"interpretation pilot lane mismatch: {key}={lane_by_doc.get(key, 0)}")
    if dict(source_counts) != EXPECTED_SOURCE_COUNTS:
        raise RuntimeError(f"interpretation pilot source split mismatch: {dict(source_counts)}")
    if dict(target_counts) != TARGET_LABEL_COUNTS:
        raise RuntimeError(f"interpretation pilot target label mismatch: {dict(target_counts)}")
...
def write_seed_preflight_copy(seed_rows: list[dict[str, str]]) -> None:
    ...
    source_counts = Counter(row["source_subset"] for row in seed_rows)
    target_counts = Counter(row["target_correct_choice"] for row in seed_rows)
    tail_counts = Counter(row.get("interpretation_failure_class", "") for row in seed_rows)
    lines = [
        ...
        f"- interpretation_failure_counts: `{dict(tail_counts)}`",
        "",
        "## checks",
        "| check | result |",
        "| --- | --- |",
...
## interpretation repair pilot 추가 지시
- 이번 run은 `해석례_QA` response-scope / answer-uniqueness repair pilot이다.
...
- 후처리 validator가 target label `{seed.get('target_correct_choice', '')}`로 choice를 재배치하므로, 생성 단계에서는 label 위치보다 정답 유일성, 회답 범위 제한, 원문 기반성에 집중한다.
"""
    return messages
...
def choose_interpretation_validator_action(
    row: dict[str, str],
    ...
    if nearmiss_score == 3 or tags & {"오답약함", "near_miss_부족", "형식 부적합"}:
        return "audit", "interpretation_quality_audit", ["weak_or_form_audit"]
...
def row_gate_fields(row: dict[str, str], answer_match_count: int) -> dict[str, str]:
    tags = validator_replay.split_tags(row.get("error_tags", ""))
    answer_unique = answer_match_count == 1 and not tags & {"정답 비유일", "오답이 정답 가능"}
    scope_limited = not tags & {"복수 쟁점 혼합", "원문 외 사실 추가"}
    source_only = not tags & {"원문 외 사실 추가", "근거부족"}
    # 기존 `same_direction_distractor`는 `예`가 통과 신호라 오해 여지가 있어 명시적 ok alias를 함께 둔다.
    same_direction_blocked = not tags & {"정답 비유일", "오답이 정답 가능"}
    return {
        "answer_uniqueness": "예" if answer_unique else "아니오",
        "condition_preservation": "아니오" if "복수 쟁점 혼합" in tags else "예",
        "response_scope_limited": "예" if scope_limited else "아니오",
        "answer_reason_split": "아니오" if "복수 쟁점 혼합" in tags else "예",
        "source_only_fact": "예" if source_only else "아니오",
        "distractor_direction": "one_axis_or_clean" if same_direction_blocked else "same_direction_blocked",
        "same_direction_distractor": "예" if same_direction_blocked else "아니오",
        "same_direction_guard_ok": "예" if same_direction_blocked else "아니오",
    }


def failure_class_from_row(row: dict[str, str], gate_fields: dict[str, str], reason_short: str) -> str:
    if reason_short in {"answer_uniqueness_tag", "single_correct_choice_failed"} or gate_fields.get("answer_uniqueness") == "아니오":
        return "answer_uniqueness"
    if gate_fields.get("response_scope_limited") == "아니오":
        return "response_scope"
    if gate_fields.get("source_only_fact") == "아니오":
        return "source_only_fact"
    if gate_fields.get("same_direction_distractor") == "아니오":
        return "same_direction_distractor"
    if reason_short in {"weak_or_form_audit", "nearmiss_score_le_2"}:
        return "weak_distractor"
    if row.get("final_status") == "soft_fail":
        return "soft_tail"
    if row.get("final_status") == "hard_fail":
        return "hard_tail"
    return "clean_or_upstream_audit"


def apply_interpretation_validator(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in rows:
        ...
        for field in ("interpretation_axis", "interpretation_seed_action", "interpretation_risk_flags", "interpretation_failure_class"):
    ...
    for row in selected:
        ...
        action, status, reasons = choose_interpretation_validator_action(row, match_count)
        ...
        failure_class = failure_class_from_row(row, gate_fields, reason_short)
...
def write_validator_artifacts(report_rows: list[dict[str, str]], rows: list[dict[str, str]]) -> None:
    ...
    tail_counts = Counter(row["interpretation_failure_class"] for row in report_rows)
    tail_usable = Counter(row["interpretation_failure_class"] for row in report_rows if row["train_eligible"] == "예")
    ...
    answer_uniqueness_recurrence_count = sum(
        ...
        or row["answer_uniqueness"] == "아니오"
        or row["response_scope_limited"] == "아니오"
        or row["source_only_fact"] == "아니오"
        or row["same_direction_distractor"] == "아니오"
    )
    ...
    VALIDATOR_SUMMARY = {
        ...
        "interpretation_failure_counts": dict(tail_counts),
        "interpretation_failure_usable_counts": dict(tail_usable),
...
def write_validator_report_md(report_rows: list[dict[str, str]]) -> None:
    lines = [
        ...
        f"- target_failure_recurrence_count: `{VALIDATOR_SUMMARY.get('answer_uniqueness_recurrence_count', 0)}`",
        ...
        "",
        "## row actions",
        "| seed | source | lane | interpretation_axis | failure_class | action | disposition | reason | split | count | target | export | status | audit | eligible |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in report_rows:
        lines.append(
            f"| `{row['seed_sample_id']}` | `{row['source_subset']}` | `{row['sampling_lane']}` | `{row['interpretation_axis']}` | `{row.get('failure_class', '')}` | `{row['validator_action']}` | `{row['validator_export_disposition']}` | `{row['validator_reason_short']}` | `{row['split_allowed']}` | `{row['count_allowed']}` | `{row['target_correct_choice']}` | `{row['export_correct_choice']}` | `{row['final_status']}` | `{row['audit_required']}` | `{row['train_eligible']}` |"
        )
...
def write_pilot_breakout(report_rows: list[dict[str, str]]) -> None:
    ...
    axis_counts = Counter(row["interpretation_axis"] for row in report_rows)
    axis_usable = Counter(row["interpretation_axis"] for row in report_rows if row["train_eligible"] == "예")
    axis_audit = Counter(row["interpretation_axis"] for row in report_rows if row["audit_required"] == "예")
    axis_hard = Counter(row["interpretation_axis"] for row in report_rows if row["final_status"] == "hard_fail")
    axis_soft = Counter(row["interpretation_axis"] for row in report_rows if row["final_status"] == "soft_fail")
    for axis in sorted(axis_counts):
        rows.append(
            {
                "breakout_type": "interpretation_axis",
                "key": axis,
                "seed_count": str(axis_counts[axis]),
                "train_eligible": str(axis_usable[axis]),
                "audit": str(axis_audit[axis]),
                "hard_fail": str(axis_hard[axis]),
                "soft_fail": str(axis_soft[axis]),
            }
        )

    lane_counts = Counter(row["sampling_lane"] for row in report_rows)
    lane_usable = Counter(row["sampling_lane"] for row in report_rows if row["train_eligible"] == "예")
    lane_audit = Counter(row["sampling_lane"] for row in report_rows if row["audit_required"] == "예")
    lane_hard = Counter(row["sampling_lane"] for row in report_rows if row["final_status"] == "hard_fail")
    lane_soft = Counter(row["sampling_lane"] for row in report_rows if row["final_status"] == "soft_fail")
    for lane in sorted(lane_counts):
        rows.append(
            {
                "breakout_type": "sampling_lane",
                "key": lane,
                "seed_count": str(lane_counts[lane]),
                "train_eligible": str(lane_usable[lane]),
                "audit": str(lane_audit[lane]),
                "hard_fail": str(lane_hard[lane]),
                "soft_fail": str(lane_soft[lane]),
            }
        )

    tail_counts = Counter(row["failure_class"] for row in report_rows)
    tail_usable = Counter(row["failure_class"] for row in report_rows if row["train_eligible"] == "예")
    tail_audit = Counter(row["failure_class"] for row in report_rows if row["audit_required"] == "예")
    tail_hard = Counter(row["failure_class"] for row in report_rows if row["final_status"] == "hard_fail")
    tail_soft = Counter(row["failure_class"] for row in report_rows if row["final_status"] == "soft_fail")
    for tail_class in sorted(tail_counts):
        rows.append(
            {
                "breakout_type": "failure_class",
                "key": tail_class,
    ...
    lines.extend(["", "## interpretation axis", "| key | seed | train_eligible | audit | hard_fail | soft_fail |", "| --- | ---: | ---: | ---: | ---: | ---: |"])
    for row in rows:
        if row["breakout_type"] == "interpretation_axis":
            lines.append(
                f"| `{row['key']}` | `{row['seed_count']}` | `{row['train_eligible']}` | `{row['audit']}` | `{row['hard_fail']}` | `{row['soft_fail']}` |"
            )
    lines.extend(["", "## sampling lane", "| key | seed | train_eligible | audit | hard_fail | soft_fail |", "| --- | ---: | ---: | ---: | ---: | ---: |"])
    for row in rows:
        if row["breakout_type"] == "sampling_lane":
            lines.append(
                f"| `{row['key']}` | `{row['seed_count']}` | `{row['train_eligible']}` | `{row['audit']}` | `{row['hard_fail']}` | `{row['soft_fail']}` |"
            )
    lines.extend(["", "## failure class", "| key | seed | train_eligible | audit | hard_fail | soft_fail |", "| --- | ---: | ---: | ---: | ---: | ---: |"])
    for row in rows:
        if row["breakout_type"] == "failure_class":
            lines.append(
                f"| `{row['key']}` | `{row['seed_count']}` | `{row['train_eligible']}` | `{row['audit']}` | `{row['hard_fail']}` | `{row['soft_fail']}` |"
            )
...
def split_dataset_with_interpretation_validator(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    if rows:
        pb6.pb4.pb3.base.write_csv_atomic(RAW_MERGED_BEFORE_VALIDATOR_PATH, rows, list(rows[0].keys()))
    validated_rows = apply_interpretation_validator(rows)
    if validated_rows:
...
def status_fields() -> dict[str, str]:
    return {
        ...
        "count_disposition": COUNT_REFLECTION_STATUS,
            ...
            enriched.update(
                {
                    ...
                    "interpretation_axis": source.get("interpretation_axis", ""),
                    "interpretation_seed_action": source.get("interpretation_seed_action", ""),
                    "interpretation_failure_class": source.get("interpretation_failure_class", ""),
                }
            )
...
def write_manifest_header_gate(manifest_rows: list[dict[str, str]]) -> None:
    ...
    if missing:
        raise RuntimeError(f"interpretation pilot manifest required fields missing: {missing}")
    lines = [
...
def build_tail_memo(merged_rows: list[dict[str, str]]) -> list[dict[str, str]]:
        ...
        tail_rows = [
            {
                ...
                "interpretation_failure_class": "tail 없음",
                "final_status": "",
                "audit_required": "",
                "validator_action": "",
                "validator_export_disposition": "",
    ...
    lines = [
        ...
        "",
        "| seed | source | lane | status | audit | validator | disposition | reason | interpretation_failure | error_tags |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in tail_rows:
        lines.append(
            f"| `{row['seed_sample_id']}` | `{row['source_subset']}` | `{row['sampling_lane']}` | `{row['final_status']}` | `{row['audit_required']}` | `{row['validator_action']}` | `{row['validator_export_disposition']}` | `{row['validator_reason_short']}` | `{row['interpretation_failure_class']}` | `{row['error_tags']}` |"
        )
    ...
    manifest.update(
        {
            ...
            "candidate_recipe_source": "v2_difficulty_patch_r2_interpretation_repair_pilot",
            "seed_registry_strategy": "same_16_seed_registry_as_objective_interpretation_repair_pilot_seed_preflight",
            "current_count_decision": "not_counted_until_reviewer_signoff_after_api_pilot",
            "count_reflection_status": COUNT_REFLECTION_STATUS,
            "downstream_consumption_allowed": DOWNSTREAM_CONSUMPTION_ALLOWED,
            "manifest_artifact_sync_status": "synced",
            "manifest_artifact_sync_note": "All top-level counts and artifact paths point to this interpretation repair pilot artifacts.",
...
def configure_interpretation_pilot_globals() -> None:
    ...
    pb6.RUN_LABEL = "objective interpretation repair API pilot"
    pb6.SEED_ID_PREFIX = "interpretation_repair_pilot"
    ...
    pb6.CANDIDATE_RECIPE_SOURCE = "v2_difficulty_patch_r2_interpretation_repair_pilot"
    pb6.SEED_REGISTRY_STRATEGY = "fixed_from_objective_interpretation_repair_pilot_seed_preflight"
    pb6.LAW_STATUS_NOTE = "interpretation_repair_pilot_not_counted_until_signoff"
    ...
    pb6.pb4.pb3.base.split_dataset = split_dataset_with_interpretation_validator


def main() -> dict:
    configure_interpretation_pilot_globals()
    return pb6.main()
